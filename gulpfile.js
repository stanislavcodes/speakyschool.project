...
function scssTask() {
  return src(files.scssPath, { sourcemaps: true }) // set source and turn on sourcemaps
    ...
    .pipe(concat("style.css")) // combine all SCSS to one style.css
...
function watchTask() {
  watch(
    [files.scssPath, files.jsPath, files.pugPathWatch],
    { interval: 1000, usePolling: true }, //Makes docker work
    series(parallel(pugTask, scssTask, jsTask), cacheBustTask)
  );
}
